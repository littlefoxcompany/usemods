...
test("escapeHtml", () => {
  expect(escapeHtml("<p>Hello world</p>")).toBe(
    "&lt;p&gt;Hello world&lt;/p&gt;"
  )
})
...
test("title", () => {
  expect(
    title("A new welcome for my brand new test for titles in javascript!")
  ).toBe("A New Welcome for My Brand New Test for Titles in Javascript!")
})
...
test("list", () => {
  expect(list(["Hello", "World"], "ol")).toStrictEqual(
    "<ol><li>Hello</li><li>World</li></ol>"
  )
})

test("commaList", () => {
  expect(commaList(["Apple", "Oranges", "Grapes", "Bread", "Milk"])).toBe(
    "Apple, Oranges, Grapes, Bread and Milk"
  )
  expect(commaList(["Apple", "Oranges", "Grapes", "Bread"], "or")).toBe(
    "Apple, Oranges, Grapes or Bread"
  )
})

test("truncateList", () => {
  expect(truncateList(["Apple", "Oranges", "Grapes", "Bread", "Milk"], 3)).toBe(
    "Apple, Oranges, Grapes and 2 more"
  )
  expect(
    truncateList(["Apple", "Oranges", "Grapes", "Bread", "Milk"], 3, "or")
  ).toBe("Apple, Oranges, Grapes or 2 more")
})

test("shuffle", () => {
  expect(shuffle(["Apple", "Oranges", "Grapes", "Bread", "Milk"])).not.toBe([
    "Apple",
    "Oranges",
    "Grapes",
    "Bread",
...
test("unique", () => {
  ...
  expect(unique([{ id: 1 }, { id: 2 }, { id: 1 }], "id")).toStrictEqual([
    { id: 1 },
    { id: 2 },
  ])
...
test("surround", () => {
  ...
  expect(surround("www.helloworld.com", "www.", ".com")).toBe(
    "www.helloworld.com"
  )
})
...
test("combine", () => {
  ...
  expect(combine([1, 2, 3], [2, 3, 4], [3, 4, 5])).toStrictEqual([
    1, 2, 3, 2, 3, 4, 3, 4, 5,
  ])
})
...
test("combineUnique", () => {
  ...
  expect(combineUnique([1, 2, 3], [2, 3, 4], [3, 4, 5])).toStrictEqual([
    1, 2, 3, 4, 5,
  ])
})
