<template>
  <main>
    ...
    <!-- Pages -->
    <div class="container relative mx-auto flex max-md:flex-col min-h-screen py-12 gap-12">
      <nav class="md:-mt-2 w-full md:w-2/12 flex flex-col md:sticky h-fit top-6">
        <div
          v-for="section in sidebar"
          class="pb-12 flex flex-col first:text-xl"
        >
          <NuxtLink
      ...
      <!-- Content -->
      <div class="min-h-screen w-full md:w-7/12">
        <NuxtPage class="w-full" />

        <!-- Jagger Swagger -->
        <section
          v-if="route.fullPath === '/docs/actions'"
          id="jagger"
          class="flex bg-indigo-900 h-96 overflow-clip rounded-2xl mt-24 items-center justify-center relative"
        >
          <Button
            class="z-10 relative shadow-black shadow-2xl"
