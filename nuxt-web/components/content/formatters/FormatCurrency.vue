<template>
  <Example>
    <ExampleInputs>
      <FormInput label="Number" type="number" v-model="value" />
    ...
    <ExampleCode :code="`formatCurrency(${value}, { ${decimals ? `decimals: ${decimals},` : ''} locale: ${locale} })`"/>
    <ExampleResult>
      {{ formatCurrency(value, { ...(decimals ? { decimals } : {}), locale }) }}
    </ExampleResult>
  </Example>
</template>
