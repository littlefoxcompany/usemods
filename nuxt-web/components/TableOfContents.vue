<template>
  <nav class="text-gray-900 dark:text-white">
    ...
    <ul>
      <li v-for="item in data" :key="item.id" class="mt-0">
        <NuxtLink
          v-if="item.id"
          ...
          <div
            ...
            :class="activeSections.includes(item.id) ? ' border-indigo-600 dark:border-amber-400 ' : 'border-gray-200 dark:border-white/15'"></div>
          <div class="truncate py-1.5" :class="activeSections.includes(item.id) ? 'text-indigo-600 dark:text-amber-400' : 'text-gray-500 dark:text-white/50'">
            {{ item.title }}
          </div>
        </NuxtLink>
